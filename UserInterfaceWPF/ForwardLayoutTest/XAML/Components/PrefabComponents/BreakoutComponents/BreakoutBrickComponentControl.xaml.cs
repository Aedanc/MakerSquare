...
{
    [Serializable]
    public class BreakoutBrickComponentSerializedData : ISerializedComponent
    {       
        public string brick_sprite;
        public int brick_score;

        public void SetComponentData(Entity entity)
        {
            var breakout = new BreakoutBrickComponentControl(entity);
            entity.AddComponent(breakout);
        }
    }

    public partial class BreakoutBrickComponentControl : UserControl, IComponent
    {
        ...
        public BreakoutBrickComponentControl(Entity entity)
        {
            InitializeComponent();
        ...
        {
            var data = new BreakoutBrickComponentSerializedData();
            data.brick_sprite = spriteName;
            data.brick_score = Int32.Parse(ScoreTextBox.Text);
            entity._serializedComponents.Add(data);
        }

        public void FillData()
        {
            var data = new FFBreakoutBrickComponent(selectedEntity);
            data.spritename = spriteName;
            data.score = Int32.Parse(ScoreTextBox.Text);
            engineData.Add(data);
        }
